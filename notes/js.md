...
- In JS *function* is not just a function it is also object.
...
But Promises return a Promise object which has *value* which will be assigned when promise completes
and *onFulfilled* array which will keep track of what should be done with returend value.
The functions in *onFulfilled* will be added when using *.then* method.
...
But here the *Microtask Queue* has higher priority that *Callback Queue*
hence the promise's data will be handled first.
...
- *Object.create* if passed any blueprint object it creates a new object with prototype of argument (blueprint object) that is passed to it.
It adds a hidden *\_\_proto\_\_* property which has blueprint of the passed in object,
It creates a *__Prototype Chain__* it is *\_\_proto\_\_* of blueprint -> *\_\_proto\_\_* of *Object.prototype* -> *null*.
